...
\begin{definition}
	...
	If $\iota$ is an embedding, then $\iota(S)$ is an embedded \emph{submanifold}\index{submanifold}. If $m = n - 1$, we call it a hypersurface.
...
\begin{definition}
	...
	\]
	The \emph{boundary}\index{boundary} of $M$, $\partial M$, is the set of points mapped in any chart to $\{x^1 = 0\}$. It is naturally an $n-1$ dimensional manifold with embedding $\iota : \partial M \to M$.
...
The \emph{unit normal}\index{unit normal} to $S$ is the unique unit vector $\vec n$ orthogonal to $\iota_\ast b_2, \ldots, \iota_\ast b_n$ with
\[
